...
return {
  ...
  },
  {
    ...
    config = function()
      ...
      vim.keymap.set({ 'n', 'x', 'o' }, ']m', function() mov.goto_next_start('@class.outer', 'textobjects') end)
      vim.keymap.set({ 'n', 'x', 'o' }, '[f', function() mov.goto_previous_start('@function.outer', 'textobjects') end)
      vim.keymap.set({ 'n', 'x', 'o' }, '[m', function() mov.goto_previous_start('@class.outer', 'textobjects') end)

      -- Swap text objects
      local sw = require 'nvim-treesitter-textobjects.swap'
      vim.keymap.set({ 'n', 'x', 'o' }, '<leader>sp', function() sw.swap_next({ '@parameter.inner' }, 'textobjects') end)
      vim.keymap.set({ 'n', 'x', 'o' }, '<leader>sP', function() sw.swap_previous({ '@parameter.inner' }, 'textobjects') end)
    end,
  },
}
